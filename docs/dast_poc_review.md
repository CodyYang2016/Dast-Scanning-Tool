...
An evidence-based funding review of NationwideDevin/ssd-dast-tool-poc at commit 7c16c01, covering requirements traceability, design-vs-code gaps, DAST maturity, UX, scan quality, and the demo story.
...
Evidence base: repository `NationwideDevin/ssd-dast-tool-poc` at commit `7c16c01` ("Phase 2 demo stuff"), read in full (all 15 Python modules, 21 test files, 18 docs, contracts, compose/Containerfile, demo scripts). Test suite executed locally. No live ZAP/Juice Shop run was performed in this review, so all runtime numbers quoted are the project's own documented measurements and are labeled as such.
...
* **Demo artifacts are text only.** There are no screenshots, images, or recordings in the repository — every "demo artifact" is Markdown plus Mermaid. The GitHub upload, the most stakeholder-legible moment, is explicitly flagged as a "one-way door … so today I'm showing" a narration rather than a live push (`docs/dast_poc_phase2_demo_runbook.md` line 433).
...
| FR-X2 GitHub Security tab | `github_upload_design.md` | `github_upload.py` shells out to `gh api`; no token/API path, no automation | Usually **narrated, not run** ("one-way door") | Partially implemented |
...
**Vulnerability detection and scan gating**
...
Requirements §9 step 4, still undemoed
...
2. **Nothing runs unattended** — no CI workflow at all; SARIF upload is a manual `gh` call usually narrated rather than executed.
...
* An explicit build-vs-buy position: what this layer adds on top of ZAP/Burp/StackHawk, and what it deliberately will not do.
...
Funding recommendation: continue, at Phase-3 scale, with the scope re-pointed from "more discovery" to "make the loop unattended and the findings actionable". The riskiest open question is not technical — it is whether the differentiated value (governed authoring, honest lifecycle) is large enough to justify building around a free scanner rather than buying one and keeping only the safety and lifecycle layers. That decision should be made explicitly at the next gate, not deferred again.

*Sources: full read of the repository at commit 7c16c01 (all Python modules, contracts, tests, compose/Containerfile, and 18 documents in docs/), plus a local run of the test suite (228 passed). Runtime figures (page/API counts, detection counts, scan durations) are the project's own documented measurements from docs/dast_poc_phase2_demo_deck.md and were not independently reproduced here; no live ZAP or Juice Shop scan was executed for this review. Statements about GitHub code scanning rendering behavior are marked as inference where not verified against the pilot repository. No product code was modified.*
